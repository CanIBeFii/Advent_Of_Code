...
fn check_quantities(quantities: &HashMap<SueTypeInfo, u8>, info: (SueTypeInfo, u8)) -> bool {
	match info.0 {
		SueTypeInfo::Children => info.1 == quantities[&info.0],
		SueTypeInfo::Cats => info.1 > quantities[&info.0],
		SueTypeInfo::Samoyeds => info.1 == quantities[&info.0],
		SueTypeInfo::Pomeranians => info.1 < quantities[&info.0],
		SueTypeInfo::Akitas => info.1 == quantities[&info.0],
		SueTypeInfo::Vizslas => info.1 == quantities[&info.0],
		SueTypeInfo::Goldfish => info.1 < quantities[&info.0],
		SueTypeInfo::Trees => info.1 > quantities[&info.0],
		SueTypeInfo::Cars => info.1 == quantities[&info.0],
		SueTypeInfo::Perfumes => info.1 == quantities[&info.0],
	}
}
...
fn part_2(input: &str) -> usize {
	...
	for (index, line) in input {
		...
		if !check_quantities(&infos_quantities, info) {
			continue;
		}
...
Sue 2: goldfish: 10, trees: 1, akitas: 0
...
Sue 9: vizslas: 0, cats: 8, trees: 4
